...
export const Delete_TermRelationsOneDeep = (firstLevel:any, secondLevel:any, collectionName: any, fieldUpdateName: any): CollectionAfterDeleteHook => async (args) => {
    const {
        id,
        doc,
        req: { payload },
    ...
    // Handle arrays
    var toDelete;
    if (Array.isArray(doc?.[firstLevel]?.map((a:any) => a[secondLevel])?.filter((a:any) => a)?.[0])) {
        var hasEntry = doc?.[firstLevel]?.map((a:any) => a[secondLevel]).filter((a:any) => a);
        toDelete = hasEntry?.map((a:any) => a?.map((b:any) => b.id)).flat();
    }
    else { 
        toDelete = doc?.[firstLevel]?.map((a: any) => a[secondLevel]?.id);
    }
