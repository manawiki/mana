...
export const Update_TermRelationsOneDeep =
   (
      firstLevel: any,
      secondLevel: any,
      collectionName: any,
   ...
   async (args) => {
      ...
      const newDataIds = data?.[firstLevel]
         ?.map((a: any) => a[secondLevel])
         .flat();
      const oldDataIds = previousDoc?.[firstLevel]
         ?.map((a: any) => a[secondLevel])
         .flat();

      const toDelete = oldDataIds?.filter((x: any) => !newDataIds?.includes(x));
