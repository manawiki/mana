...
import { _SkillClassificationSpecifics } from "../collections/_skill-classification-specifics";
...
const collectionName = "_skill-classification-specifics";
const data = require("./import_files/" + "_SkillClassificationSpecific" + ".json");
...
const fieldConfig = _SkillClassificationSpecifics.fields;
...
const seedUploads = async (result: any) => {
	...
	var iconImport:any = {}
	fieldConfig.filter(fc => fc.type == "upload").map((f:any) => {
		iconImport[f.name] = result[f.name]?.name.replace(".png","");
	});

	// Array Fields with relations:
	var arrayData: any = [];
	var arraysWithRelation = fieldConfig.filter(fc => fc.type == "array")?.map((f:any) => {
		return {
			name: f.name,
			fields: f.fields?.filter((fac:any) => fac.type == "relationship" ).map((fa:any) => {
				return {
					name: fa.name,
					collection: fa.relationTo,
					relationIdField: result[f.name]?.find(r => r[fa.name])?.[fa.name] ? Object.keys(result[f.name]?.find(r => r[fa.name])?.[fa.name])?.[0] : null, // Find Id field for if an entry has the relation
					hasMany: fa.hasMany
				}
			})
		}
		...
		if (result[awr.name]?.length > 0) {
			returnArrayElement = result[awr.name].map((resulte:any) => {
				...
				for (var arfield in awr.fields) {
					...
					else if (!arrayRelField.hasMany && arrayRelField.relationIdField) {
						tempobj[arrayRelField.name] = resulte?.[arrayRelField.name]?.[arrayRelField.relationIdField].toString(); // Substitute Identifier out. At some point will need to write some code that will handle the case where the relationIdField's value is NOT the same as the id field, where a lookup of entry by relationIdField is done first.
					}

					// For multi relationship fields in an array, NON Polymorphic
					else if (arrayRelField.hasMany && arrayRelField.relationIdField && resulte?.[arrayRelField.name]?.[arrayRelField.relationIdField] && !Array.isArray(arrayRelField.collection)) {
						tempobj[arrayRelField.name] = resulte?.[arrayRelField.name]?.[arrayRelField.relationIdField]?.map((multirelation:any) => multirelation.toString()); // Substitute Identifier out. At some point will need to write some code that will handle the case where the relationIdField's value is NOT the same as the id field, where a lookup of entry by relationIdField is done first.
					}

					// For multi relationship fields in an array, POLYMORPHIC
					else if (arrayRelField.hasMany && arrayRelField.relationIdField && resulte?.[arrayRelField.name]?.[arrayRelField.relationIdField] && Array.isArray(arrayRelField.collection)) {
						tempobj[arrayRelField.name] = resulte?.[arrayRelField.name]?.[arrayRelField.relationIdField]?.map((polymorphic:any) => {
							return {
	...
	const relationFields = fieldConfig.filter(fc => fc.type == "relationship" && !fc.hasMany).map((f:any) => {
		return {
			name: f.name,
			collection: f.relationTo,
			relationIdField: result[f.name] ? Object.keys(result[f.name])?.[0]: null
		}
	})
	...
	const relationArray = relationFields.map((field:any) => {
		if (result[field.name]) {
			return {
				[field.name]: result[field.name]?.[field.relationIdField]?.toString()
			}
		}
		else {
			return null;
