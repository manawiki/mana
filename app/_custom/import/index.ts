...
const data = require("./import_files/termElement.json");
...
const seedUploads = async (result: any) => {
   ...
   if (existingEntry.docs.length > 0) {
      ...
      var custData = {
         ...
         icon_active:
            siteId + "_" + result.icon_active?.name.replace(".png", ""),
         icon_inactive:
            siteId + "_" + result.icon_inactive?.name.replace(".png", ""),
         icon_damage_res:
            siteId + "_" + result.icon_damage_res?.name.replace(".png", ""),
      };
   ...
   // Otherwise, create a new entry
      ...
      console.log(
         `${JSON.stringify(createItemCustom)} Custom Data Import completed!`
      );
   }
};
