...
const seedUploads = async (result: any) => {
        ...
        fields: f.fields
          ...
          .map((fa: any) => {
            return {
              ...
              relationIdField: result[f.name]?.find((r: any) => r[fa.name])?.[
                fa.name
              ]
                ? Object.keys(
                    result[f.name]?.find((r: any) => r[fa.name])?.[fa.name]
                  )?.[0]
          ...
          // For multi relationship fields in an array, POLYMORPHIC
            ...
            tempobj[arrayRelField.name] = resulte?.[arrayRelField.name]?.[
              ...
              if (!polymorphic.relationTo) {
                console.log(
                  "ERROR: Polymorphic Relation Fields require 'relationTo' and 'value' key values!"
                );
              }
              return {
  ...
  if (existingEntry.docs.length > 0) {
    if (result.checksum == existingEntry.docs[0].checksum && !forceOverwrite) {
