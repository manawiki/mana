...
import { NoblePhantasms } from "../collections/noble-phantasms";
...
const collectionName = "noble-phantasms";
const data = require("./import_files/" + "NoblePhantasm" + ".json");
...
const fieldConfig = NoblePhantasms.fields;
...
const seedUploads = async (result: any) => {
   ...
   const relationFields = fieldConfig
      .filter((fc) => fc.type == "relationship")
      .map((f: any) => {
         return {
            name: f.name,
